...
//        std::string correlations_folder = "./takahashi_nonsq_GM_sq7_RF_ell120_iZ_Mss_U70W75W75_cross_zs10_zs16_mc_2e6_x2_220_20000_with_baryons_change_params_bin_averaged_P_qag/";
        ...
        //double zs1 = 3.5; // KiDS - Pierre
        //double zs2 = 3.5; // KiDS - Pierre
        ...
        //std::vector<std::vector<double>> nofz_s1_table = read_2_column_table("../data/nofz/Kids_nofz/nofz_KiDS1000_bin4-f1.dat");
        //assert(!nofz_s1_table.empty());
        //normalise_n_of_z(nofz_s1_table);
        //Linear_interp_1D nofz_s1(nofz_s1_table.at(0), nofz_s1_table.at(1));

        //std::vector<std::vector<double>> nofz_s2_table = read_2_column_table("../data/nofz/Kids_nofz/nofz_KiDS1000_bin5-f1.dat");
        //assert(!nofz_s2_table.empty());
        //normalise_n_of_z(nofz_s2_table);
        ...
        std::vector<double> B_sss_lower_limit = { zs_lower };
        std::vector<double> B_sss_upper_limit = { zs_upper };
...
//        double zl_upper = 0.36;
        ...
        std::vector<std::vector<double>> n_of_z_table = read_2_column_table("../data/nofz/pofz_projected_density_maps.tab");
        //std::vector<std::vector<double>> n_of_z_table = read_2_column_table("../data/nofz/Kids_nofz/nofz_LRG_bin2_coarse-f1.dat");
        assert(!n_of_z_table.empty());
        normalise_n_of_z(n_of_z_table);

        //for (size_t idx = 0; idx < n_of_z_table.at(0).size(); idx++)
        //     std::cout << n_of_z_table.at(0).at(idx) << "     " << n_of_z_table.at(1).at(idx) << std::endl;

        Linear_interp_1D n_of_z(n_of_z_table.at(0), n_of_z_table.at(1));

        std::shared_ptr<projection_kernel> qg1(new projection_kernel_q_m(class_obj.get(), &n_of_z));
        ...
        filename_B = "B_kkk.dat"; // B_kkk kappa equilateral bispectrum

        //std::string B_integration_algorithm = "qag";
        //std::string B_integration_algorithm = "mc";
        std::string B_integration_algorithm = "hcubature";
    ...
    //                std::cout << l_array.at(i) << " " << l_array.at(j) << " " << l_array.at(k) << " " << iB_phi_1_phi_2_lF2_array[idx] << std::endl;
        ...
        */
            ...
            for (size_t idx = 0; idx < l_array.size(); idx++)
            {
                    ...
                    for (size_t a = 0; a < zs_bins.size() ; a++)
                    {
                        for (size_t b = a; b < zs_bins.size() ; b++)
                        {
                            ...
                            if (P_integration_algorithm == "qag")
                                P_ss_array[0][corr_idx][idx] = P_los_qag("P", l_array.at(idx), class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(), P_ss_lower_limit.at(0), P_ss_upper_limit.at(0));

                            else if (P_integration_algorithm == "mc")
                                P_ss_array[0][corr_idx][idx] = P_los_mc("P", l_array.at(idx), class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(), P_ss_lower_limit, P_ss_upper_limit, T, "vegas");

                            else if (P_integration_algorithm == "hcubature")
                                P_ss_array[0][corr_idx][idx] = P_los_hcubature("P", l_array.at(idx), class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(), P_ss_lower_limit, P_ss_upper_limit);
                ...
                else if (filename_P == "P_hh.dat")
                {
                    ...
                    for (size_t a = 0; a < zl_bins.size() ; a++) // only auto-P for lens bins
                    {
                        ...
                        {
                            // b1 term
                            P_ll_array[0][corr_idx][idx] = P_los_qag("P", l_array.at(idx), class_obj.get(), use_pk_nl, ql_b1_kernels.at(a).get(), ql_b1_kernels.at(a).get(), P_ll_lower_limit.at(0), P_ll_upper_limit.at(0));

                            // Shot noise term
                            P_ll_array[1][corr_idx][idx] = P_los_qag("P_hh_eps_eps", l_array.at(idx), class_obj.get(), use_pk_nl, ql_kernels.at(a).get(), ql_kernels.at(a).get(), P_ll_lower_limit.at(0), P_ll_upper_limit.at(0));
                        }

                        else if (P_integration_algorithm == "mc")
                        {
                            // b1 term
                            P_ll_array[0][corr_idx][idx] = P_los_mc("P", l_array.at(idx), class_obj.get(), use_pk_nl, ql_b1_kernels.at(a).get(), ql_b1_kernels.at(a).get(), P_ll_lower_limit, P_ll_upper_limit, T, "vegas");

                            // Shot noise term
                            P_ll_array[1][corr_idx][idx] = P_los_mc("P_hh_eps_eps", l_array.at(idx), class_obj.get(), use_pk_nl, ql_kernels.at(a).get(), ql_kernels.at(a).get(), P_ll_lower_limit, P_ll_upper_limit, T, "vegas");
                        }

                        else if (P_integration_algorithm == "hcubature")
                        {
                            // b1 term
                            P_ll_array[0][corr_idx][idx] = P_los_hcubature("P", l_array.at(idx), class_obj.get(), use_pk_nl, ql_b1_kernels.at(a).get(), ql_b1_kernels.at(a).get(), P_ll_lower_limit, P_ll_upper_limit);

                            // Shot noise term
                            P_ll_array[1][corr_idx][idx] = P_los_hcubature("P_hh_eps_eps", l_array.at(idx), class_obj.get(), use_pk_nl, ql_kernels.at(a).get(), ql_kernels.at(a).get(), P_ll_lower_limit, P_ll_upper_limit);
                        }
        ...
        if (compute_2D_bispectra_equilateral)
        {
            ...
            std::vector<std::vector<std::vector<double>>> B_sss_array;

            B_sss_array = std::vector<std::vector<std::vector<double>>>(1, std::vector<std::vector<double>>(num_3pt_sss_correlations, std::vector<double>(l_array.size(), 0)));
                    ...
                    for (size_t a = 0; a < zs_bins.size() ; a++)
                    {
                        for (size_t b = a; b < zs_bins.size() ; b++)
                        {
                            for (size_t c = b; c < zs_bins.size() ; c++)
                            {
                                ...
                                if (B_integration_algorithm == "qag")
                                    B_sss_array[0][corr_idx][l_idx] = B_los_qag("B", l_array.at(l_idx), l_array.at(l_idx), l_array.at(l_idx), class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(), qs_kernels.at(c).get(), B_sss_lower_limit.at(0), B_sss_upper_limit.at(0));

                                else if (B_integration_algorithm == "mc")
                                    B_sss_array[0][corr_idx][l_idx] = B_los_mc("B", l_array.at(l_idx), l_array.at(l_idx), l_array.at(l_idx), class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(), qs_kernels.at(c).get(), B_sss_lower_limit, B_sss_upper_limit, T, "vegas");

                                else if (B_integration_algorithm == "hcubature")
                                    B_sss_array[0][corr_idx][l_idx] = B_los_hcubature("B", l_array.at(l_idx), l_array.at(l_idx), l_array.at(l_idx), class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(),  qs_kernels.at(c).get(), B_sss_lower_limit, B_sss_upper_limit);
                ...
                for (size_t a = 0; a < zs_bins.size() ; a++)
                {
                    for (size_t b = a; b < zs_bins.size() ; b++)
                    {
                        for (size_t c = b; c < zs_bins.size() ; c++)
                        {
                            ...
                            for (size_t l_idx = 0; l_idx < l_array.size(); l_idx++)
                                file_B << l_array.at(l_idx) << " " << B_sss_array.at(0).at(corr_idx).at(l_idx) << "\n";
        ...
        if (compute_2D_integrated_bispectra)
        {
            ...
            struct_iB_W_FS info_iB_W_FS = {&W2D_U_FS, theta_U, &W2D_TH_FS, theta_T};
            struct_iB_W_FS info_iB_W_FS_halos = {&W2D_TH_FS, theta_T, &W2D_TH_FS, theta_T};

            if (filename_iB == "iBggg_mc.dat")
            {
                iB_P_1_val = iB_mc("P_1", 0, info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_lll_lower_limits, iB_lll_upper_limits, T, "vegas", calls_iB_initial);
                std::cout << "\niB_P_1 = " << iB_P_1_val << std::endl;

                iB_A_val = iB_mc("A", 0, info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_lll_lower_limits, iB_lll_upper_limits, T, "vegas", calls_iB_initial);
            ...
            {
                iB_P_1_val = iB_hcubature("P_1", 0, info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_lll_lower_limits, iB_lll_upper_limits, calls_iB_initial);
                std::cout << "\niB_P_1 = " << iB_P_1_val << std::endl;

                iB_A_val = iB_hcubature("A", 0, info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_lll_lower_limits, iB_lll_upper_limits, calls_iB_initial);
            ...
            for (size_t l_idx = 0; l_idx < l_array.size(); l_idx++)
            {
                ...
                {
                    iB_111[l_idx][0] = iB_mc("B", l_array.at(l_idx), info_iB_W_FS_halos, class_obj.get(), true, qh1_b1.get(), qh1_b1.get(), qh1_b1.get(), iB_lll_lower_limits, iB_lll_upper_limits, T, "vegas", calls_iB);
                    iB_111[l_idx][1] = iB_mc("B_PP", l_array.at(l_idx), info_iB_W_FS_halos, class_obj.get(), true, qh1_b1.get(), qh1_b1.get(), qh1_b2.get(), iB_lll_lower_limits, iB_lll_upper_limits, T, "vegas", calls_iB);
                }

                if (filename_iB == "iBhhh_hcubature.dat") // with bias
                {
                    iB_111[l_idx][0] = iB_hcubature("B", l_array.at(l_idx), info_iB_W_FS_halos, class_obj.get(), true, qh1_b1.get(), qh1_b1.get(), qh1_b1.get(), iB_lll_lower_limits, iB_lll_upper_limits, calls_iB);
                }

                if (filename_iB == "iBgkk_hcubature.dat")
                {
                     // without bias
                    iB_111[l_idx][0] = iB_hcubature("B_lF2", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qk1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][3] = iB_hcubature("Z_W", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qk1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][6] = iB_hcubature("Z_S2", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qk1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                }

                else if (filename_iB == "iBggk_hcubature.dat")
                {
                     // without bias
                    iB_111[l_idx][0] = iB_hcubature("B_lF2", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][2] = iB_hcubature("Y_W", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][3] = iB_hcubature("Z_W", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][5] = iB_hcubature("Y_S2", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][6] = iB_hcubature("Z_S2", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][9] = iB_hcubature("P_3", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                }

                else if (filename_iB == "iBggg_hcubature.dat")
                {
                     // without bias
                    iB_111[l_idx][0] = iB_hcubature("B_lF2", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][1] = iB_hcubature("X_W", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][2] = iB_hcubature("Y_W", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][3] = iB_hcubature("Z_W", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][4] = iB_hcubature("X_S2", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][5] = iB_hcubature("Y_S2", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][6] = iB_hcubature("Z_S2", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][7] = iB_P_1_val;
                    iB_111[l_idx][8] = iB_hcubature("P_2", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][9] = iB_hcubature("P_3", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][10] = iB_A_val;
                }

                if (filename_iB == "iBkkk_hcubature.dat")
                {
                    iB_111[l_idx][0] = iB_hcubature("B", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);

                    if (do_cross_fields)
                    {
                        iB_112[l_idx][0] = iB_hcubature("B", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                        iB_122[l_idx][0] = iB_hcubature("B", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                        iB_222[l_idx][0] = iB_hcubature("B", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                   }
                }

                else if (filename_iB == "iBkxi_hcubature.dat")
                {
                    iB_111[l_idx][0] = iB_hcubature("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][1] = iB_hcubature("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
        //            iB_111[idx][2] = iB_hcubature("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_lower_limits, iB_upper_limits, calls_iB);
        //            iB_111[idx][3] = iB_hcubature("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_lower_limits, iB_upper_limits, calls_iB);

                    if (do_cross_fields)
                    {
                        iB_112[l_idx][0] = iB_hcubature("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                        iB_112[l_idx][1] = iB_hcubature("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
        //                iB_112[idx][2] = iB_hcubature("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);
        //                iB_112[idx][3] = iB_hcubature("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);

                        iB_122[l_idx][0] = iB_hcubature("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                        iB_122[l_idx][1] = iB_hcubature("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
        //                iB_122[idx][2] = iB_hcubature("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);
        //                iB_122[idx][3] = iB_hcubature("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);

                        iB_222[l_idx][0] = iB_hcubature("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                        iB_222[l_idx][1] = iB_hcubature("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
        //                iB_222[idx][2] = iB_hcubature("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);
        //                iB_222[idx][3] = iB_hcubature("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);
                    }
                }

                else if (filename_iB == "iBkxi_hcubature_v.dat")
                {
                    iB_111[l_idx][0] = iB_hcubature_v("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][1] = iB_hcubature_v("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
        //            iB_111[idx][2] = iB_hcubature_v("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_lower_limits, iB_upper_limits, calls_iB);
        //            iB_111[idx][3] = iB_hcubature_v("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_lower_limits, iB_upper_limits, calls_iB);

                    if (do_cross_fields)
                    {
                        iB_112[l_idx][0] = iB_hcubature_v("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                        iB_112[l_idx][1] = iB_hcubature_v("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
        //                iB_112[idx][2] = iB_hcubature_v("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);
        //                iB_112[idx][3] = iB_hcubature_v("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);

                        iB_122[l_idx][0] = iB_hcubature_v("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                        iB_122[l_idx][1] = iB_hcubature_v("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
        //                iB_122[idx][2] = iB_hcubature_v("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);
        //                iB_122[idx][3] = iB_hcubature_v("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);

                        iB_222[l_idx][0] = iB_hcubature_v("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                        iB_222[l_idx][1] = iB_hcubature_v("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
        //                iB_222[idx][2] = iB_hcubature_v("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);
        //                iB_222[idx][3] = iB_hcubature_v("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);
                    }
                }

                else if (filename_iB == "iBkxi_hcubature_angle_averaged.dat")
                {
                    iB_111[l_idx][0] = iB_hcubature_angle_averaged("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                    iB_111[l_idx][1] = iB_hcubature_angle_averaged("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
        //            iB_111[idx][2] = iB_hcubature_angle_averaged("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_lower_limits, iB_upper_limits, calls_iB);
        //            iB_111[idx][3] = iB_hcubature_angle_averaged("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_lower_limits, iB_upper_limits, calls_iB);

                    if (do_cross_fields)
                    {
                        iB_112[l_idx][0] = iB_hcubature_angle_averaged("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                        iB_112[l_idx][1] = iB_hcubature_angle_averaged("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
        //                iB_112[idx][2] = iB_hcubature_angle_averaged("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);
        //                iB_112[idx][3] = iB_hcubature_angle_averaged("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);

                        iB_122[l_idx][0] = iB_hcubature_angle_averaged("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                        iB_122[l_idx][1] = iB_hcubature_angle_averaged("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
        //                iB_122[idx][2] = iB_hcubature_angle_averaged("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);
        //                iB_122[idx][3] = iB_hcubature_angle_averaged("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);

                        iB_222[l_idx][0] = iB_hcubature_angle_averaged("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
                        iB_222[l_idx][1] = iB_hcubature_angle_averaged("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, calls_iB);
        //                iB_222[idx][2] = iB_hcubature_angle_averaged("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);
        //                iB_222[idx][3] = iB_hcubature_angle_averaged("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, calls_iB);
                    }
                }

                else if (filename_iB == "iBkxi_mc.dat")
                {
                    iB_111[l_idx][0] = iB_mc("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, T, "vegas", calls_iB);
                    iB_111[l_idx][1] = iB_mc("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_sss_lower_limits, iB_sss_upper_limits, T, "vegas", calls_iB);
        //            iB_111[idx][2] = iB_mc("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_lower_limits, iB_upper_limits, T, "vegas", calls_iB);
        //            iB_111[idx][3] = iB_mc("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk1.get(), iB_lower_limits, iB_upper_limits, T, "vegas", calls_iB);

                    if (do_cross_fields)
                    {
                        iB_112[l_idx][0] = iB_mc("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, T, "vegas", calls_iB);
                        iB_112[l_idx][1] = iB_mc("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, T, "vegas", calls_iB);
        //                iB_112[idx][2] = iB_mc("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_lower_limits, iB_upper_limits, T, "vegas", calls_iB);
        //                iB_112[idx][3] = iB_mc("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk1.get(), qk2.get(), iB_lower_limits, iB_upper_limits, T, "vegas", calls_iB);

                        iB_122[l_idx][0] = iB_mc("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, T, "vegas", calls_iB);
                        iB_122[l_idx][1] = iB_mc("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, T, "vegas", calls_iB);
        //                iB_122[idx][2] = iB_mc("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, T, "vegas", calls_iB);
        //                iB_122[idx][3] = iB_mc("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk1.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, T, "vegas", calls_iB);

                        iB_222[l_idx][0] = iB_mc("B_xip_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, T, "vegas", calls_iB);
                        iB_222[l_idx][1] = iB_mc("B_xim_cos", l_array.at(l_idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_sss_lower_limits, iB_sss_upper_limits, T, "vegas", calls_iB);
        //                iB_222[idx][2] = iB_mc("B_xip_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, T, "vegas", calls_iB);
        //                iB_222[idx][3] = iB_mc("B_xim_sin", l_array.at(idx), info_iB_W_FS, class_obj.get(), true, qk2.get(), qk2.get(), qk2.get(), iB_lower_limits, iB_upper_limits, T, "vegas", calls_iB);
    ...
    //        double iB_P_1_val = 0, iB_A_val = 0;
            ...
            struct_iB_W_FS info_iB_UWW_FS = {&W2D_U_FS, theta_U, &W2D_TH_FS, theta_T};
            struct_iB_W_FS info_iB_WWW_FS = {&W2D_TH_FS, theta_T, &W2D_TH_FS, theta_T};

    //        if (filename_iB == "iB_ggg_mc.dat")
    //        {
    //            iB_P_1_val = iB_mc("P_1", 0, info_iB_WWW_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_source_lower_limits, iB_source_upper_limits, T, "vegas", calls_iB_initial);
    //            std::cout << "\niB_P_1 = " << iB_P_1_val << std::endl;

    //            iB_A_val = iB_mc("A", 0, info_iB_WWW_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_source_lower_limits, iB_source_upper_limits, T, "vegas", calls_iB_initial);
    ...
    //        {
    //            iB_P_1_val = iB_hcubature("P_1", 0, info_iB_WWW_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_source_lower_limits, iB_source_upper_limits, calls_iB_initial);
    //            std::cout << "\niB_P_1 = " << iB_P_1_val << std::endl;

    //            iB_A_val = iB_hcubature("A", 0, info_iB_WWW_FS, class_obj.get(), true, qg1.get(), qg1.get(), qg1.get(), iB_source_lower_limits, iB_source_upper_limits, calls_iB_initial);
                        ...
                        for (size_t b = a; b < zs_bins.size() ; b++)
                        {
                            for (size_t c = b; c < zs_bins.size() ; c++)
                            {
                                ...
                                if (iB_integration_algorithm == "mc")
                                {
                                    ...
                                    iB_mc("B", l_array.at(l_idx), info_iB_UWW_FS, class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(), qs_kernels.at(c).get(), iB_sss_lower_limits, iB_sss_upper_limits, T, "vegas", result, error, calls_iB);
                                ...
                                if (iB_integration_algorithm == "mc_cigar")
                                {
                                    // iB term
                                    result = 0.0, error = 0.0;
                                    iB_mc_cigar("B", l_array.at(l_idx), info_iB_UWW_FS, class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(), qs_kernels.at(c).get(), iB_sss_lower_limits, iB_sss_upper_limits, cigar, result, error, thread_count);
                                    iB_sss_array[0][corr_idx][l_idx] = result;
                                    iB_sss_error_array[0][corr_idx][l_idx] = error;
                                }
                                ...
                                else if (iB_integration_algorithm == "hcubature")
                                {
                                    // iB term
                                    result = 0.0, error = 0.0;
                                    iB_hcubature("B", l_array.at(l_idx), info_iB_UWW_FS, class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(), qs_kernels.at(c).get(), iB_sss_lower_limits, iB_sss_upper_limits, result, error, calls_iB);
                                    iB_sss_array[0][corr_idx][l_idx] = result;
                                    iB_sss_error_array[0][corr_idx][l_idx] = error;
                                }
                        ...
                        for (size_t b = a; b < zs_bins.size() ; b++)
                        {
                            for (size_t c = b; c < zs_bins.size() ; c++)
                            {
                                ...
                                if (iB_integration_algorithm == "mc")
                                {
                                    // iBp term
                                    result = 0.0, error = 0.0;
                                    iB_mc("B_xip_cos", l_array.at(l_idx), info_iB_UWW_FS, class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(), qs_kernels.at(c).get(), iB_sss_lower_limits, iB_sss_upper_limits, T, "vegas", result, error, calls_iB);
                                    iB_sss_array[0][corr_idx][l_idx] = result;
                                    iB_sss_error_array[0][corr_idx][l_idx] = error;
                                ...
                                if (iB_integration_algorithm == "mc_cigar")
                                {
                                    // iBp term
                                    result = 0.0, error = 0.0;
                                    iB_mc_cigar("B_xip_cos", l_array.at(l_idx), info_iB_UWW_FS, class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(), qs_kernels.at(c).get(), iB_sss_lower_limits, iB_sss_upper_limits, cigar, result, error, thread_count);
                                    iB_sss_array[0][corr_idx][l_idx] = result;
                                    iB_sss_error_array[0][corr_idx][l_idx] = error;

                                    // iBm term
                                    result = 0.0, error = 0.0;
                                    iB_mc_cigar("B_xim_cos", l_array.at(l_idx), info_iB_UWW_FS, class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(), qs_kernels.at(c).get(), iB_sss_lower_limits, iB_sss_upper_limits, cigar, result, error, thread_count);
                                    iB_sss_array[1][corr_idx][l_idx] = result;
                                    iB_sss_error_array[1][corr_idx][l_idx] = error;
                                }
                                ...
                                else if (iB_integration_algorithm == "hcubature")
                                {
                                    // iBp term
                                    result = 0.0, error = 0.0;
                                    iB_hcubature("B_xip_cos", l_array.at(l_idx), info_iB_UWW_FS, class_obj.get(), use_pk_nl, qs_kernels.at(a).get(), qs_kernels.at(b).get(), qs_kernels.at(c).get(), iB_sss_lower_limits, iB_sss_upper_limits, result, error, calls_iB);
                                    iB_sss_array[0][corr_idx][l_idx] = result;
                                    iB_sss_error_array[0][corr_idx][l_idx] = error;
                        ...
                        for (size_t b = 0; b < zs_bins.size() ; b++)
                        {
                            for (size_t c = b; c < zs_bins.size() ; c++)
                            {
                                ...
                                if (iB_integration_algorithm == "mc")
                                {
                                    // b1 term
                                    result = 0.0, error = 0.0;
                                    iB_mc("B", l_array.at(l_idx), info_iB_WWW_FS, class_obj.get(), use_pk_nl, ql_b1_kernels.at(a).get(), qs_kernels.at(b).get(), qs_kernels.at(c).get(), iB_lll_lower_limits, iB_lll_upper_limits, T, "vegas", result, error, calls_iB);
                                    iB_lss_array[0][corr_idx][l_idx] = result;
                                    iB_lss_error_array[0][corr_idx][l_idx] = error;

                                    // b2 term
                                    result = 0.0, error = 0.0;
                                    iB_mc("B_P2P3", l_array.at(l_idx), info_iB_WWW_FS, class_obj.get(), use_pk_nl, ql_b2_kernels.at(a).get(), qs_kernels.at(b).get(), qs_kernels.at(c).get(), iB_lll_lower_limits, iB_lll_upper_limits, T, "vegas", result, error, calls_iB);
                                    iB_lss_array[1][corr_idx][l_idx] = result;
                                    iB_lss_error_array[1][corr_idx][l_idx] = error;

                                    // bs2 term
                                    result = 0.0, error = 0.0;
                                    iB_mc("B_S2P2P3", l_array.at(l_idx), info_iB_WWW_FS, class_obj.get(), use_pk_nl, ql_bs2_kernels.at(a).get(), qs_kernels.at(b).get(), qs_kernels.at(c).get(), iB_lll_lower_limits, iB_lll_upper_limits, T, "vegas", result, error, calls_iB);
                ...
                else if (filename_iB == "iB_hhh.dat")
                {
                    ...
                    for (size_t a = 0; a < zl_bins.size() ; a++)
                    {
                        ...
                        if (iB_integration_algorithm == "mc")
                        {
                            // b1 term
                            result = 0.0, error = 0.0;
                            iB_mc("B", l_array.at(l_idx), info_iB_WWW_FS, class_obj.get(), use_pk_nl, ql_b1_kernels.at(a).get(), ql_b1_kernels.at(a).get(), ql_b1_kernels.at(a).get(), iB_lll_lower_limits, iB_lll_upper_limits, T, "vegas", result, error, calls_iB);
                            iB_lll_array[0][corr_idx][l_idx] = result;
                            iB_lll_error_array[0][corr_idx][l_idx] = error;

                            // b2 term
                            result = 0.0, error = 0.0;
                            iB_mc("B_PP", l_array.at(l_idx), info_iB_WWW_FS, class_obj.get(), use_pk_nl, ql_b1_kernels.at(a).get(), ql_b1_kernels.at(a).get(), ql_b2_kernels.at(a).get(), iB_lll_lower_limits, iB_lll_upper_limits, T, "vegas", result, error, calls_iB);
                            iB_lll_array[1][corr_idx][l_idx] = result;
                            iB_lll_error_array[1][corr_idx][l_idx] = error;

                            // bs2 term
                            result = 0.0, error = 0.0;
                            iB_mc("B_S2PP", l_array.at(l_idx), info_iB_WWW_FS, class_obj.get(), use_pk_nl, ql_b1_kernels.at(a).get(), ql_b1_kernels.at(a).get(), ql_bs2_kernels.at(a).get(), iB_lll_lower_limits, iB_lll_upper_limits, T, "vegas", result, error, calls_iB);
                            iB_lll_array[2][corr_idx][l_idx] = result;
                            iB_lll_error_array[2][corr_idx][l_idx] = error;

                            // Shot noise 1 term
                            result = 0.0, error = 0.0;
                            iB_mc("B_hhh_delta_eps_eps", l_array.at(l_idx), info_iB_WWW_FS, class_obj.get(), use_pk_nl, ql_b1_kernels.at(a).get(), ql_b1_kernels.at(a).get(), ql_kernels.at(a).get(), iB_lll_lower_limits, iB_lll_upper_limits, T, "vegas", result, error, calls_iB);
                            iB_lll_array[3][corr_idx][l_idx] = result;
                            iB_lll_error_array[3][corr_idx][l_idx] = error;

                            // Shot noise 2 term
                            result = 0.0, error = 0.0;
                            iB_mc("B_hhh_eps_eps_eps", l_array.at(l_idx), info_iB_WWW_FS, class_obj.get(), use_pk_nl, ql_kernels.at(a).get(), ql_kernels.at(a).get(), ql_kernels.at(a).get(), iB_lll_lower_limits, iB_lll_upper_limits, T, "vegas", result, error, calls_iB);
