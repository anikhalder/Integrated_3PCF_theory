...
//        std::string correlations_folder = "./takahashi_nonsq_GM_sq7_RF_ell120_iZ_Mss_U70W75W75_cross_zs10_zs16_mc_2e6_x2_220_20000_with_baryons_change_params_bin_averaged_P_qag/";
        ...
        std::string iB_l_z_folder = "./iB_l_z_U70W75W75_bsr_nonsq_bihalofit_sq7_NA_ell120_z50_mc_4dim_5e5/";
        std::string spectra_folder = "./takahashi_bsr_nonsq_bihalofit_sq7_NA_iB_Mss_U70W75W75_cross_zs10_zs16_mc_4dim_5e5_trapz_iB_l_z_v2_P_mc/";
        std::string correlations_folder = "./takahashi_bsr_nonsq_bihalofit_sq7_NA_iZ_Mss_U70W75W75_cross_zs10_zs16_mc_4dim_5e5_trapz_iB_l_z_v2_P_mc/";
        ...
        {
            double z = 0 + i*delta_z;

            double q_1 = qs_kernels.at(0).get()->evaluate(z);
            double q_2 = qs_kernels.at(1).get()->evaluate(z);
        ...
        if (compute_2D_2PCF)
        {
            ...
            if (filename_P == "P_kk.dat")
            {
                ...
                for (size_t a = 0; a < zs_bins.size() ; a++)
                {
                    for (size_t b = a; b < zs_bins.size() ; b++)
                    {
                        ...
                        std::vector<std::vector<double>> P_matrix = read_n_column_table(P_flat_2D_spectrum, 2);
            ...
            else if (filename_P == "P_hh.dat" || filename_P == "P_gg.dat")
            {
                ...
                for (size_t a = 0; a < zl_bins.size() ; a++)
                {
                    ...
                    std::vector<std::vector<double>> P_matrix = read_n_column_table(P_flat_2D_spectrum, 3);
    ...
    //        }
            ...
            */
            ...
            if (filename_iB == "iB_kkk.dat" || filename_iB == "iB_Mkk.dat")
            {
                ...
                assert (iB_l_z_grid.size() == l_array.size());
                assert (iB_l_z_grid.at(0).size() == z_array.size());
            }
            ...
            // write projected spectra to files
            if (filename_iB == "iB_kkk.dat" || filename_iB == "iB_Mkk.dat" || filename_iB == "iB_Mss.dat")
            {
                size_t corr_idx = 0;
                for (size_t a = 0; a < zs_bins.size() ; a++)
                {
        ...
        if (compute_2D_integrated_3PCF)
        {
                ...
                for (size_t a = 0; a < zs_bins.size() ; a++)
                {
                    for (size_t b = a; b < zs_bins.size() ; b++)
                    {
                        for (size_t c = b; c < zs_bins.size() ; c++)
                        {
                            ...
                            std::vector<std::vector<double>> iB_Re_matrix = read_n_column_table(iB_flat_2D_spectrum, 3);
                ...
                for (size_t a = 0; a < zl_bins.size() ; a++)
                {
                    for (size_t b = 0; b < zs_bins.size() ; b++)
                    {
                        for (size_t c = b; c < zs_bins.size() ; c++)
                        {
                            ...
                            std::vector<std::vector<double>> iB_matrix = read_n_column_table(iB_flat_2D_spectrum, 7);
            ...
            if (filename_iB == "iB_hhh.dat") // with bias
            {
                ...
                for (size_t a = 0; a < zl_bins.size() ; a++)
                {
                    ...
                    std::vector<std::vector<double>> iB_matrix = read_n_column_table(iB_flat_2D_spectrum, 6);
