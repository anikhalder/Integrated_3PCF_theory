...
public:
    ...
    bihalofit(ClassEngine *class_obj); // parameterised constructor for flat wCDM model using ClassEngine
    ~bihalofit();
    ...
    // sigma
    double window(double x, int j);
    double sigmam(double r, int j); // Eq.(B1) 2nd part
    double calc_r_sigma(double D1); // Eq.(B1) 1st part
...
//    Linear_interp_1D pk_lin;  // TBD

    struct params_sigmam_integrand { double r; int j; bihalofit *bihalofit_obj;};
    static double sigmam_qag_integrand(double k, void *params);
    double sigmam_qag(double r, int j);
